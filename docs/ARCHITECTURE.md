...
**Transactions serialize globally.** better-sqlite3 is one synchronous connection, but callbacks are async, so
...
Things that will surprise you, roughly in order of how likely you are to hit them.

- **`initModule` is the otel/pino module registration helper in `src/server/logger.ts`**, not a general
  module-init abstraction. Nothing else in the codebase is named that.
- **There is no client-side module-init abstraction.** Each `*.client.ts` system exports its own ad hoc
  `setup()`, called in order from `main.tsx`. `LayerDataClient.setup()` is awaited at the top level before
  anything renders, because layer components are read synchronously throughout the component tree.
- **Frame instance lookup is a linear scan with a deep-equal per entry**, not a hash lookup. Fine at current
  instance counts, a scaling cliff if frame counts grow.
- **`spanOp`'s ctx detection is heuristic.** It checks `args[0]`, `args[0][0]`, and `args[0][last]`. A
  spanOp'd function whose ctx sits elsewhere silently loses attribute and link propagation, while `opts.attrs`
  keeps working, so the failure looks like partial instrumentation rather than an error.
- **`packId` has no overflow check or versioning.** A large enough future roster of factions/units could
  silently overflow into adjacent bit fields.
- **RBAC checks are hand-rolled per handler**, not middleware, so you cannot tell whether a handler is
  permission-checked without reading it.
- **`migrate.ts` uses `BEGIN IMMEDIATE` to detect a busy database**, which cannot actually detect an _idle_
  app holding the file. Known bug.
- **COEP is pinned to `credentialless` on purpose.** Discord's CDN sends no CORP header, so tightening to
  `require-corp` silently kills every avatar and emoji.
- **Unsaved settings changes fire a native `beforeunload` dialog** that breaks browser automation with a
  misleading `chrome-extension://` error. Save or reset before navigating.
- **Legacy compatibility paths are load-bearing and mostly silent**: a legacy filter comparison shape is
  upgraded on read, `secret-box` retries a legacy key derivation for v1 envelopes, `fromRow` returns `null`
  rather than throwing on unparseable audit rows (an append-only log accumulates old shapes forever).
- **Some dead code is kept deliberately.** The `PLAYER_BANNED` RCON path is commented as a squadjs carryover
  with no recent occurrences in prod logs.
- **Command aliases are plain text substitution with no argument binding.** The counter-example that killed
  the general version is recorded in-code: with fixed-duration timeout aliases, the duration sits between the
  player and the reason, so there is no sound way to splice positional arguments.
- **`toStream` needs `fireImmediately`** or it silently skips the store's current value.
- **Breaking changes to anything persisted** (db, settings, localStorage, env) must be flagged explicitly to
  the user, since this is a self-hosted app with real deployments and no migration safety net beyond the
  runner.
