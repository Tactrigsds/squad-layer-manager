...
one process tree). The app ships OTLP to it over HTTP on `:4318` (`OTLP_COLLECTOR_ENDPOINT`), and
Grafana is on <http://localhost:3001>.
...
- **SLM / Ops (RED)** (`slm-ops`) is rate/errors/duration for every `C.spanOp` in the app, plus Node
  ...
  `spanOp`'s `finally` block, so **a new `spanOp` appears here with no extra wiring**.
