...
beforeAll(async () => {
	app = await createAppFixture({ layerQueue: queue(LAYERS.gorodokRaas, LAYERS.narvaRaas) })
...
describe('plugin host', () => {
	...
	it('serves the active events over the generic rpc stream', async () => {
		const first = await firstYield(
			(signal) =>
				client.plugins.rpcStream({ pluginId: 'balance-triggers', name: 'activeEvents', serverId: app.serverId, input: {} }, { signal }),
