...
describe('the server loads its layer engine lazily', () => {
	...
	// the saving only exists if it goes away again: holding it after the queries are done is the whole cost
	it('releases it once nothing has queried for a while, and reloads on the next use', async () => {
		...
		await app.waitFor(() => /reloaded the layer engine/.test(appLog()), {
			label: 'the engine reloading on next use',
			timeoutMs: 30_000,
		})
	}, 140_000)
})
