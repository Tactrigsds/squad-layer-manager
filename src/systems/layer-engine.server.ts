...
const envBuilder = Env.getEnvBuilder({ ...Env.groups.layers })
...
export function setup(): Promise<void> {
	...
	idleReleaseMs = envBuilder().LAYER_ENGINE_IDLE_RELEASE
...
function keep(engine: LayerEngine): LayerEngine {
	...
	releaseTimer = setTimeout(release, idleReleaseMs).unref()
	return engine
}
...
function release() {
	...
	forceGc?.()
	log.debug('released the layer engine after %dms idle', idleReleaseMs)
}
