	...
	return {
		...
		events: {
			...
			PLAYER_ADDED_TO_TEAM: {
				...
				chainID: 549,
				raw: '',
				playerIds: { username: 'Test Player' },
				teamId,
			},
		},
	}
...
async function syncUp(state: PendingEvents.State, opts: { layerId?: L.LayerId; teams?: SM.Teams } = {}): Promise<SE.Event[]> {
...
describe('PendingEvents', () => {
	...
	describe('roll layer resolution', () => {
		// an in-game vote overrides the layer SLM set, so expectedNewLayerId misses and the roll falls back to rcon.
		// rcon can still be reporting the outgoing layer at that moment, which recorded the finished match as the one
		// starting -- a duplicate history row for the layer that just played.
		it('refuses a polled layer that contradicts the loading layer', async () => {
			const { state } = makeState({ layerId: LAYER_A })
			await syncUp(state, { layerId: LAYER_A })
			state.nextLayerId = LAYER_A
			state.hooks.fetchLayersStatus = vi.fn().mockResolvedValue({ currentLayer: { id: LAYER_A } })
			const onRoll = vi.fn().mockResolvedValue({ match: makeMatchDetails(3, LAYER_B), nextLayerId: null })
			state.hooks.onNewGameDuringRoll = onRoll
			...
			// the server is bringing up Narva, but the poll still says Gorodok
			PendingEvents.onLogEvent(state, {
				type: 'NEW_GAME',
				time: 1100,
				chainID: 0,
			...
			const events = await collectAt(state, 1200)

			expect(onRoll).not.toHaveBeenCalled()
			expect(events.some((e) => e.type === 'NEW_GAME')).toBe(false)
			expect(state.syncState.type).toBe('rolling')
		})

		it('accepts a polled layer that agrees with the loading layer', async () => {
			const { state } = makeState({ layerId: LAYER_A })
			await syncUp(state, { layerId: LAYER_A })
			state.nextLayerId = null
			state.hooks.fetchLayersStatus = vi.fn().mockResolvedValue({ currentLayer: { id: LAYER_B } })
			const onRoll = vi.fn().mockResolvedValue({ match: makeMatchDetails(3, LAYER_B), nextLayerId: null })
			state.hooks.onNewGameDuringRoll = onRoll
			...
			expect(onRoll).toHaveBeenCalledWith(LAYER_B, 1100)
	...
	describe('TEAMS_UPDATE reconciliation (complex simultaneous changes)', () => {
		...
		it('two solo players switch teams simultaneously, each disbanding their own squad', async () => {
			...
			PendingEvents.onTeamsPolled(state, makeTeams([makePlayer('eos-001', 2), makePlayer('eos-002', 2)], []), 500)
			const events = await collect(state)
		...
		it('mixed: team switch triggers squad disband with mid-process leader succession, plus role change on another player', async () => {
			...
			PendingEvents.onTeamsPolled(
				state,
				makeTeams([makePlayer('eos-001', 2), makePlayer('eos-002', 1, { role: 'Rifleman_01' }), makePlayer('eos-003', 2)], []),
				500,
			)
			const events = await collect(state)
	...
	describe('ordering invariants', () => {
		...
		it('emitted event IDs are strictly ascending within a process() call', async () => {
			...
			PendingEvents.onTeamsPolled(
				state,
				makeTeams([makePlayer('eos-001', 1), makePlayer('eos-002', 1, { squadId: 1, isLeader: true })], [sq1]),
				200,
			)
			const events = await collect(state)
		...
		it('emitted event IDs are strictly ascending across consecutive process() calls', async () => {
			...
			PendingEvents.onTeamsPolled(state, makeTeams([makePlayer('eos-001', 1), makePlayer('eos-002', 1, { squadId: 1 })], [sq1]), 200)
			const batch1 = await collect(state)
	...
	describe('MAP_SET attribution', () => {
		...
		it('is not poisoned by a stale attribution for a different layer', async () => {
			const state = syncedState()
			PendingEvents.pushAttribution(state, {
				type: 'MAP_SET_ATTRIBUTION',
				itemId: 'stale',
				layerId: LAYER_B,
				appEventId: 'app-stale',
			})
			PendingEvents.pushAttribution(state, { type: 'MAP_SET_ATTRIBUTION', itemId: 'item-1', layerId: LAYER_A, appEventId: 'app-1' })
			mapSetLog(state, LAYER_A_CLASSNAME, 'RGF+CombinedArms ADF+Mechanized')
