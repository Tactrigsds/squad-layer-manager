...
describe('LogEvents.parse', () => {
	...
	describe('non-chain events', () => {
		...
		// Verbatim from the 2026-07-21 02:53 roll: the destination layer came up and the first player
		// joined in the same engine tick, so the NEW_GAME shared chainID 244 with a join chain.
...
// Real engine-tick groups pulled from the log archive by src/scripts/scan-chain-straddles.ts, weighted towards
// the ticks where a chain shares its chainID with something else. The parser used to keep only the chain and
// discard the rest of the tick, which lost the NEW_GAME of a map roll whenever a player joined on the load tick.
...
describe('LogEvents.parse conservation', () => {
	...
	// the invariant the tick partition exists to hold: a recognized log entry is never silently discarded
