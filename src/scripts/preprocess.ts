...
async function parsePipelineData() {
    return await fsPromise.readFile(path.join(PROJECT_ROOT, 'src', 'assets', 'squad-pipeline.json'), 'utf8').then(data => SquadPipelineModels.PipelineOutputSchema.parse(JSON.parse(pipelineData)))
}
