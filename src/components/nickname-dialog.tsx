	...
	return (
		<Dialog modal open={props.open} onOpenChange={props.onOpenChange}>
			...
			<DialogContent className="sm:max-w-md">
				{/* DialogContent unmounts on close, and the key covers the user loading in while it is already open, so
				    the field is seeded from the current nickname on every open without an effect syncing it */}
				<NicknameForm key={user?.nickname ?? ''} onOpenChange={props.onOpenChange} />
			</DialogContent>
		</Dialog>
	)
}

function NicknameForm(props: { onOpenChange?: (newState: boolean) => void }) {
	const user = useLoggedInUser()
