...
function getMissingIndicatorFields(entity: F.FilterEntity, kind: 'match' | 'miss'): string[] {
	...
	return [!entity.invertedEmoji && 'miss emoji', !entity.invertedAlertMessage && 'miss alert message'].filter(
		(v) => typeof v === 'string',
	)
}
		...
		<div className="space-y-3">
			<span className="flex items-center gap-1">
				...
				<HelpTooltip label="About the pool filter">
					<p>
						Out-of-pool layers are hidden behind the pool toggle during layer selection, and only users with the
						queue:force-write permission can queue them. Saving one warns the editor, and in-game admins are warned when one is
						about to be played. Autogenerated layers always come from the pool.
					</p>
					<p>
						The toggle in front of the filter flips it between including its matching layers in the pool and excluding them from
						it.
					</p>
					<p>
						The filter's match indicators (emoji and alert message, plus the inverted pair for misses) are what mark a layer as
						in or out of the pool across the app, so the pool filter needs all of them configured.
					</p>
				</HelpTooltip>
			</span>
						...
						<div key={entry.filterId} className="flex items-center gap-2">
							...
							>
								<Button
									variant="ghost"
									disabled={!!api.writeDenied}
									className="h-7 grow justify-start gap-1 px-1 font-normal"
								>
									{emoji ? (
										<EmojiDisplay size="sm" emoji={emoji} />
									) : (
										<Icons.Filter className="h-4 w-4 text-orange-400" />
									)}
		...
		<section aria-label="Skip warnings for" className="space-y-2">
			<span className="flex items-center gap-1">
				...
				<HelpTooltip label="About skipping warnings">
					<p>
						A queue item carrying any of these tags raises no warnings: none in the save dialog, and none in the admin reminder
						sent before it is played.
					</p>
			...
			<div className="space-y-3">
				<span className="flex items-center gap-1">
					...
					<HelpTooltip label="About secondary filters">
						<p>
							Secondary filters never decide what is in the pool; they add behavior on top of it: displaying match or miss
							indicators on layers, being offered during layer selection, warning when a matching layer is queued or about to
							be played, and further constraining autogeneration.
						</p>
					...
					<div className="contents text-sm font-medium text-muted-foreground">
						...
						<div>
							<HelpTooltip label="About repeat rule warnings" trigger="Warn">
								<p>
									Warn the editor before saving a layer that violates this rule, and in-game admins when one is about to
									be played
								</p>
							</HelpTooltip>
						</div>
