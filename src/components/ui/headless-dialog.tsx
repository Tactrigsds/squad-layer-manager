...
>(({ className, children, showCloseButton = true, ...props }, ref) => {
	...
	return (
		<>
			<DialogBackdrop
				className="fixed inset-0 z-50 bg-black/80 transition-opacity duration-300 ease-out data-[closed]:opacity-0 data-[enter]:opacity-100"
				transition
			/>
			<div className="fixed inset-0 z-50 flex w-screen items-center justify-center p-4">
				...
				>
					{children}
					{showCloseButton && (
						<CloseButton className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[hover]:bg-accent data-[hover]:text-muted-foreground">
