// The SLM mark, as geometry. dev_docs/brand.md is the spec; every rendition of the logo (the nav bar tile, the
...
// Outlines rather than text because the favicon renders where the webfont does not load; dev_docs/brand.md says
...
export type Shape = { role: Role; d: string; translate?: { x: number; y: number } }

export function shapes(withAccent: boolean): Shape[] {
	const block = withAccent ? CAP_HEIGHT + ACCENT_GAP + ACCENT_HEIGHT : CAP_HEIGHT
	const top = (VIEW_BOX - block) / 2
	const out: Shape[] = [
		{ role: 'tile', d: roundedRect(0, 0, VIEW_BOX, VIEW_BOX, CORNER_RADIUS) },
		{ role: 'letters', d: LETTERS, translate: { x: VIEW_BOX / 2, y: top } },
	]
	if (withAccent) {
...
function path(shape: Shape, fillAttr: string) {
	const transform = shape.translate ? ` transform="translate(${shape.translate.x} ${shape.translate.y})"` : ''
	return `<path ${fillAttr}${transform} d="${shape.d}"/>`
}
