    ...
    return await tracer.startActiveSpan(
      ...
      async (span) => {
        ...
        try {
          ...
          const logLevel =
            spanStatus.code === Otel.SpanStatusCode.ERROR
...
export type SquadRcon = CS.Ctx & { server: SquadRconSys.SquadRcon } & Rcon &
  ServerId;
...
export type SquadServer = CS.Ctx &
  Rcon & {
    server: SquadServerSys.SquadServer;
  } & ServerId;

export type SharedLayerList = CS.Ctx &
...
export function durableSub<T, O>(
  ...
  return (o) => {
    const numDownstreamFailureBeforeErrorPropagation =
      opts.numOfUpstreamErrorsBeforePropagation ?? 10;
