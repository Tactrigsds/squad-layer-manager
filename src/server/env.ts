...
Leave it empty if you have no battlemetrics org: the integration turns itself off (see BM_ENABLED) and the features that read it report it as unconfigured.`,
	...
	squadbrowser: {
		// resolved from SQB_API_KEY and SQB_HOST when left unset, in ensureEnvSetup
		SQB_ENABLED: z
			.stringbool()
			.default(false)
			.meta({
				description:
					'disables the squad browser integration entirely (the dashboard join button is hidden). Defaults to off when there is no SQB_API_KEY and SQB_HOST is the real api, since there is nothing to authenticate with.',
				envExample: { include: 'commented', dev: { include: 'commented' } },
			}),

		SQB_HOST: z.url().prefault(SQUADBROWSER_API).meta({
		...
		SQB_API_KEY: z
			...
			.meta({
				...
				envExample: { include: 'set' },
				description:
					"squad browser API key, which starts with `sqb_`. It resolves a server's name into the join link behind the dashboard's join button. Leave it empty if you have no key: the integration turns itself off (see SQB_ENABLED) and the button is hidden.",
			}),
	},
...
const squadbrowserIsAStub = () => (rawEnv.SQB_HOST ?? SQUADBROWSER_API) !== SQUADBROWSER_API
...
const DEMO_CONFLICTS: { keys: string[]; conflicts: (value: string) => boolean; reason: string }[] = [
	...
	},
	{
		...
		reason:
			"a demo's servers are made up, so the key would be spent looking up names the real squad browser api has never seen. A SQB_HOST pointed at a stub is accepted",
	},
	{
		keys: ['SQB_ENABLED'],
		conflicts: (value) => groups.squadbrowser.SQB_ENABLED.safeParse(value).data === true && !squadbrowserIsAStub(),
		reason: 'a demo has no real server to hand anyone a join link for. A SQB_HOST pointed at a stub is accepted',
	},
]
...
export function ensureEnvSetup() {
	...
	rawEnv.SQB_ENABLED ??= String(rawEnv.SQB_API_KEY !== undefined || squadbrowserIsAStub())
