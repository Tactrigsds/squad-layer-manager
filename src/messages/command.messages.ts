...
export const WARNS = {
	...
	help(commands: CMD.CommandConfigs, section?: string) {
		...
		const lines = listing.commands.flatMap((id) => {
			...
			const own = `[${sortedStrings.join(', ')}]${signature ? ` ${signature}` : ''}: ${GENERAL.descriptions[id]}`
			...
			const shortcuts = cmd.triggers
				...
				.map((t) => `[${CMD.formatTriggerUsage(id, t)}]: ${GENERAL.aliasDescription(CMD.describeTriggerExpansion(cmd, t))}`)
